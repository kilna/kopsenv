...
function error_and_proceed() {
  errors+=("${1}")
  echo -e "kopsenv: Test Failed: ${1}" >&2
}
...
KOPSENV_BIN_DIR=/tmp/kopsenv-test
rm -rf ${KOPSENV_BIN_DIR} && mkdir ${TFENV_BIN_DIR}
ln -s ${PWD}/bin/* ${KOPSENV_BIN_DIR}
export PATH="${KOPSENV_BIN_DIR}:${PATH}"

echo "### Test supporting symlink"
cleanup || error_and_die "Cleanup failed?!"
kopsenv install 0.8.2 || error_and_proceed "Install failed"
kopsenv use 0.8.2 || error_and_proceed "Use failed"
